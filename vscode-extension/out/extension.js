...
let lastStderr = '';
async function checkAndSetupLanggraphConfig(projectRoot) {
    ...
    const selection = await vscode.window.showInformationMessage('No "langgraph.json" configuration file was found in your workspace root. Would you like to automatically generate one?', 'Yes, generate default', 'No, cancel');
...
async function setupPrivateVenv(context) {
...
async function findPythonPath(context, projectRoot) {
    // 1. Try to get user configured custom python path from vscode settings
    const configPath = vscode.workspace.getConfiguration('langgraph-sync').get('pythonPath');
