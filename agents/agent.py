...
def WebSearchProcessorAgent(*args, **kwargs):
    return "Dummy Cover Letter"

def ImageAnalysisAgent(*args, **kwargs):
    return {
        "resume": "Dummy Resume",
        "cover_letter": "Dummy Cover Letter"
    }

def LocalGuardrails(*args, **kwargs):
    return {
        "resume": "Dummy Resume",
        "cover_letter": "Dummy Cover Letter"
    }
...
def create_agent_graph():
    ...
    def run_conversation_agent(state: AgentState) -> AgentState:
        ...
        **You:** "I'm not a doctor, but headaches and fever can have various causes, from infections to dehydration. If your symptoms persist, you should see a medical professional."
        ...
        return {
            **state,
            "output": response,
            "agent_name": "CONVERSATION_AGENT11"
        }
    ...
    workflow.add_node("CONVERSATION_AGENT11", run_conversation_agent)
    ...
    workflow.add_conditional_edges(
        ...
        lambda x: x["next"],
        {
            "CONVERSATION_AGENT": "CONVERSATION_AGENT11",
            "RAG_AGENT": "RAG_AGENT",
