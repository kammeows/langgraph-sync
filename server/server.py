...
WORKSPACE_ROOT = os.environ.get("WORKSPACE_ROOT") or os.path.join(os.path.dirname(__file__), "..")
    ...
    if os.path.exists(LANGGRAPH_JSON_PATH):
        try:
            ...
            for g_id, path_val in graphs.items():
                # path_val is usually "./agent.py:graph" or absolute "D:/path/agent.py:graph"
                # Use rsplit to avoid splitting Windows drive letter colons (e.g. D:)
                parts = path_val.rsplit(":", 1)
                file_path = parts[0]
